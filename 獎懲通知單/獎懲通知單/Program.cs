...
namespace 獎懲通知單
{
    public class Program
    {
        ...
        {
            string URL學生獎懲通知單 = "ischool/高中系統/共用/學務/學生/報表/獎懲通知單_2013";
            string URL班級獎懲通知單 = "ischool/高中系統/共用/學務/班級/報表/獎懲通知單_2013";

            string toolName = "獎懲通知單(測試版)";

            FISCA.Features.Register(URL學生獎懲通知單, arg =>
            {
            ...
            {
                Features.Invoke(URL學生獎懲通知單);
            };

            FISCA.Features.Register(URL班級獎懲通知單, arg =>
            {
            ...
            K12.Presentation.NLDPanels.Student.SelectedSourceChanged += delegate
            {
                ...
                {
                    StudentReports["報表"]["學務相關報表"][toolName].Enable = Permissions.學生獎懲通知單權限;
                }
            };
            ...
            K12.Presentation.NLDPanels.Class.SelectedSourceChanged += delegate
            {
                ...
                {
                    ClassReports["報表"]["學務相關報表"][toolName].Enable = Permissions.班級獎懲通知單權限;
                }
            };

            Catalog ribbon = RoleAclSource.Instance["學生"]["報表"];
            ribbon.Add(new RibbonFeature(Permissions.學生獎懲通知單, toolName));

            ribbon = RoleAclSource.Instance["班級"]["報表"];
            ribbon.Add(new RibbonFeature(Permissions.班級獎懲通知單, toolName));
        }
